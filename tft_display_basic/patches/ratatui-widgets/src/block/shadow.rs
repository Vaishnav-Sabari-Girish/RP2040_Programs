...
enum Effect {
    ...
    Custom(Box<dyn CellEffect>),
}

/// Helper trait that lets `Box<dyn CellEffect>` be cloned.
///
/// Blanket-implemented for any `CellEffect` that is also `Clone`.
pub trait CellEffectClone {
    fn clone_box(&self) -> Box<dyn CellEffect>;
}

impl<T> CellEffectClone for T
where
    T: 'static + CellEffect + Clone,
{
    fn clone_box(&self) -> Box<dyn CellEffect> {
        Box::new(self.clone())
    }
...
impl Clone for Box<dyn CellEffect> {
    fn clone(&self) -> Box<dyn CellEffect> {
...
pub trait CellEffect:
    fmt::Debug + Send + Sync + UnwindSafe + RefUnwindSafe + CellEffectClone
{
...
impl PartialEq for Effect {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::Custom(lhs), Self::Custom(rhs)) => core::ptr::addr_eq(
                Box::as_ref(lhs) as *const dyn CellEffect,
                Box::as_ref(rhs) as *const dyn CellEffect,
            ),
            _ => false,
        }
    }
...
impl Hash for Effect {
    fn hash<H: Hasher>(&self, state: &mut H) {
        match self {
            ...
            Self::Custom(filter) => {
                "custom".hash(state);
                ptr::hash(Box::as_ref(filter) as *const dyn CellEffect, state);
            }
        }
    }
}
...
impl Shadow {
    ...
    pub fn custom<F: CellEffect + 'static>(effect: F) -> Self {
        Self {
            effect: Effect::Custom(Box::new(effect)),
            style: Style::default(),
            offset: Offset::new(1, 1),
        }
    }
